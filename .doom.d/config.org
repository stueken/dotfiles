...
**** Agenda file switching

Switch agenda scope between personal and Jowo GTD files.

This matches the intended workflow: usually only one context is active at a
time, either personal or employer-specific.
...
Use personal GTD files as the default agenda scope on startup.
...
Define refile targets for the GTD system.

Refile is used to move tasks out of journals, inboxes, and temporary notes into
their final GTD destination files.

Unlike opening files, refile does not browse directories. It completes against
the targets listed in =org-refile-targets=. Therefore all relevant GTD files
...
:PROPERTIES:
:CREATED:  [2026-05-23 Sa 20:57]
:END:

- =SPC X=: capture
- =SPC o a a=: open agenda
- =SPC o a p=: switch to personal agenda files
- =SPC o a j=: switch to Jowo agenda files
...
- =SPC s m n=: narrow to subtree
- =SPC s m N=: widen

***** Define helper keybindings for switching agenda scope quickly.
:PROPERTIES:
:CREATED:  [2026-05-23 Sa 20:55]
:END:
