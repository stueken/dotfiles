...
Items refiled directly into a ~tasks.org~ file are inserted at the beginning
of the file. Projects and someday items keep Org's normal ordering and are
appended below their selected target heading.
...
Use globally unique Org IDs for links between Org headings.

This keeps links stable when GTD items are refiled between inbox, tasks,
projects, someday, or their archive files.

When storing a link to an Org heading, create an ID when necessary and use
that ID instead of a file/heading-based link.
...
:PROPERTIES:
:CREATED:  [2026-06-22 Mo 19:42]
:END:

Show completed or killed items from the current GTD context.

This is useful for weekly reviews, status updates, and checking what was
finished recently. The command uses the current GTD context, so switch to
personal or Jowo first.

Use:

- =SPC o a p=: personal GTD context
- =SPC o a j=: Jowo GTD context
- =SPC o a D=: show DONE/KILL items from the last N days

The completed-items view automatically starts in Org Agenda Follow Mode.
Moving through the results with ~j~ and ~k~ therefore shows the corresponding
Org entry in another window.

Useful commands inside the completed-items view:

- ~j~ / ~k~: move through completed items
- ~C-SPC~: manually show the current item in another window
- ~M-x org-agenda-follow-mode~: toggle Follow Mode manually

The command also includes existing default Org archive files such as
=tasks.org_archive=, =projects.org_archive= and =someday.org_archive=.
...
Use Org clocking to record which tasks were actively worked on.

Clocking is used as a detailed activity marker. Billing and official time
tracking remain separate.

Clock entries are stored in the ~LOGBOOK~ drawer to keep task bodies clean.

Useful commands:

- ~SPC m c i~: clock in to the current item
- ~SPC m c o~: clock out
- ~SPC m c l~: toggle the last clock
- ~SPC m c g~: go to the currently clocked item
...
Show Jowo items that were actively worked on during the most recent previously
clocked workday.

The review searches the Jowo GTD files and their archive files for the most
recent day before today containing Org CLOCK entries.

This naturally handles weekends, holidays, vacations, sickness, and other
gaps between workdays. The view is based on actual clocking activity rather
than calendar weekdays.

Each matching Org item is listed only once, even when it contains multiple
CLOCK entries for that day.

The agenda itself stays compact and does not show CLOCK or LOGBOOK lines.
Follow Mode automatically shows the corresponding Org subtree in another
window.

Use:

- ~SPC o a R~: show Jowo activity from the last clocked workday
- ~j~ / ~k~: move through the worked-on items
- ~C-SPC~: manually show the current item in another window
- ~M-x org-agenda-follow-mode~: toggle Follow Mode manually
...
Use an indirect buffer for agenda previews so that only the selected subtree
is shown.

Keep ~PROPERTIES~ and ~LOGBOOK~ drawers folded while leaving the actual task
description and notes visible. The preview has its own folding state and does
not change the visibility of the original Org buffer.
...
- =SPC X=: capture
- ~C-c C-c~: finalize the current capture
- ~C-c C-w~: finalize and immediately refile the current capture
- =SPC o a a=: open agenda
- =SPC o a p=: switch to personal GTD context
- =SPC o a j=: switch to Jowo GTD context
- =SPC o a D=: show DONE/KILL items from the last N days
- ~SPC o a R~: show Jowo activity from the last clocked workday
- ~C-SPC~: show selected item in another window from an agenda view
- ~M-x org-agenda-follow-mode~: toggle automatic agenda follow mode
- ~SPC m c i~: clock in to the current item
- ~SPC m c o~: clock out
- ~SPC m c l~: toggle the last clock
- ~SPC m c g~: go to the currently clocked item
...
- =SPC m d s=: schedule
- =SPC m d d=: deadline
- ~SPC n l~: store a link to the current location, using an ID when possible
- ~SPC m l S~: insert the last stored link
- ~SPC m I~: create an Org ID for the current heading
...
**** Temporary Doom popup workaround

Work around a Doom popup error affecting Org's fast TODO selection.

Observed on 2026-08-05 after a recent Doom upgrade:

~wrong-type-argument: consp, nil~

The error occurs in
~+popup--org-fix-popup-window-shrinking-a~ when Org calls
~split-window-vertically~ without arguments.

After a future Doom upgrade, disable this block temporarily and test
~SPC m t~. Remove the workaround when the fast TODO selection works without it.
